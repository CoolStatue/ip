...
description: Generate a self-contained, GitHub-style split-view HTML page that visually presents changes in this project Git repository. Use when asked to show, review, share, or inspect project code changes visually; compare revisions, branches, commits, or the worktree; or create an HTML diff.
---
...
4. Resolve a Python interpreter. Prefer `python`; if it is not on the Windows PATH, locate an installed `python.exe` and invoke it with PowerShell's call operator (`&`).
5. Run the bundled generator from the repository root:
...
6. Confirm the command succeeded and report the absolute path to the generated page. Do not open a browser unless the user asks.
